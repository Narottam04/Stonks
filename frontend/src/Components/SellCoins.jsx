...
const SellCoins = ({ data, modal, setModal }) => {
  ...
  useEffect(() => {
    ...
    async function coinAmount() {
      const res = await fetch(`${import.meta.env.VITE_STONKS_API_KEY}/api/user/getPurchasedStock?stockId=${data?.symbol}`);
  ...
  async function onPlaceOrder() {
    try {
      ...
      const sellStock = await fetch(`${import.meta.env.VITE_STONKS_API_KEY}/api/user/sellStock`, {
        method: "POST",
        headers: {
