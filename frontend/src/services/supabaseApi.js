...
export const supabaseApi = createApi({
  ...
  endpoints: (builder) => ({
      ...
      async queryFn(id) {
        try {
          const res = await fetch(
            `https://api-6tyd64odzq-uc.a.run.app/api/user/getPurchasedStock?id=${id}`
          );
    ...
    getWatchlistData: builder.query({
      queryFn: async (id) => {
        try {
          const res = await fetch(`https://api-6tyd64odzq-uc.a.run.app/api/user/allWatchlist?id=${id}`);
    ...
    getPortfolioCoinData: builder.query({
      queryFn: async (id) => {
        try {
          ...
          //     const request = fetch(`https://api.coingecko.comhttps://api-6tyd64odzq-uc.a.run.app/api/v3/coins/${coinId}`);
          ...
          //   return { data: [] };
          // }
          const res = await fetch(`https://api-6tyd64odzq-uc.a.run.app/api/user/allPortfolio?id=${id}`);
    ...
    getUserNetworth: builder.query({
      queryFn: async (id) => {
        try {
          const res = await fetch(`https://api-6tyd64odzq-uc.a.run.app/api/user/networth?id=${id}`);
    ...
    updateUserNetworth: builder.query({
      queryFn: async (id) => {
        try {
          const res = await fetch(`https://api-6tyd64odzq-uc.a.run.app/api/user/networth`, {
            method: "POST",
            headers: {
    ...
    getLeaderboard: builder.query({
      queryFn: async () => {
        try {
          const res = await fetch(`https://api-6tyd64odzq-uc.a.run.app/api/user/leaderboard`);
    ...
    fetchAvailableCoins: builder.query({
      queryFn: async (id) => {
        try {
          // get available coins
          const res = await fetch(`https://api-6tyd64odzq-uc.a.run.app/api/user/vusd?id=${id}`);
