...
export const supabaseApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getWatchlistData: builder.query({
      queryFn: async (id) => {
        try {
          const res = await fetch(`${import.meta.env.VITE_STONKS_API_KEY}/api/user/allWatchlist?id=${id}`);
    ...
    getPortfolioCoinData: builder.query({
      queryFn: async (id) => {
        try {
          ...
          //   return { data: [] };
          // }
          const res = await fetch(`${import.meta.env.VITE_STONKS_API_KEY}/api/user/allPortfolio?id=${id}`);
    ...
    getUserNetworth: builder.query({
      queryFn: async (id) => {
        try {
          const res = await fetch(`${import.meta.env.VITE_STONKS_API_KEY}/api/user/networth?id=${id}`);
    ...
    updateUserNetworth: builder.query({
      queryFn: async (id) => {
        try {
          const res = await fetch(`${import.meta.env.VITE_STONKS_API_KEY}/api/user/networth`, {
            method: "POST",
            headers: {
    ...
    getLeaderboard: builder.query({
      queryFn: async () => {
        try {
          const res = await fetch(`${import.meta.env.VITE_STONKS_API_KEY}/api/user/leaderboard`);
    ...
    fetchAvailableCoins: builder.query({
      queryFn: async (id) => {
        try {
          // get available coins
          const res = await fetch(`${import.meta.env.VITE_STONKS_API_KEY}/api/user/vusd?id=${id}`);
